...
listProtectedRouter.openapi(listMahasiswaOtaRoute, async (c) => {
  ...
  try {
    ...
    const where: Record<string, unknown> = {
      ...
      User: { applicationStatus: { not: "unregistered" } },
    };
        ...
        body: {
          data: mahasiswaList.map((u) => {
            ...
            return {
              ...
              type: ({ Mahasiswa: "mahasiswa", OrangTuaAsuh: "ota", Admin: "admin", Bankes: "bankes", Pengurus_IOM: "pengurus", Pewawancara: "pengurus", Guest: "pengurus" } as Record<string, "mahasiswa" | "ota" | "admin" | "bankes" | "pengurus">)[u.role] ?? "pengurus",
              name: mp?.name ?? "",
              nim: mp?.nim ?? "",
