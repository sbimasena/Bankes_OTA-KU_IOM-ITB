# Group Feature — Backend API Reference (REFACTOR)
...
- **[NEW] Pre-funded Concept:** Grup harus mengumpulkan minimum dana (pledge) Rp800.000 sebelum dapat mengajukan proposal mahasiswa atau diaktifkan oleh admin.
...
**Role:** admin  
**Syarat:** 
- Grup harus punya minimal 1 anggota
- **[CHANGED]** Total pledge semua anggota grup harus ≥ Rp800.000. Jika kurang, akan mengembalikan pesan error.
...
| `pledgeAmount` | number | **[NEW]** Nominal komitmen dana OTA (wajib diset jika `response` adalah `"accepted"`) |
...
| Field | Tipe | Keterangan |
|-------|------|------------|
| **[CHANGED]** `nim` | string | **[CHANGED]** Payload berubah dari `mahasiswaId` menjadi `nim` (Nomor Induk Mahasiswa). Hal ini membuat payload lebih mudah digunakan dari sisi UI dengan input langsung NIM |
